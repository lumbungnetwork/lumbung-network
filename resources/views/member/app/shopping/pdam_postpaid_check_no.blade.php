...
<div class="max-w-xs mx-auto">
    <div class="p-4">
        ...
        <div class="nm-convex-gray-100 rounded-2xl p-4">
            <div class="my-2 text-xs text-gray-500">Pilih Area, lalu masukkan No. Pelanggan</div>
            {{-- Order Form --}}
            <form action="{{ $formRoute }}" method="POST" id="order-form">
                @csrf
                <div class="my-3 nm-inset-gray-200 rounded-lg p-1">
                    <select class="bg-transparent text-xs font-light ml-1 focus:outline-none w-full"
                        name="buyer_sku_code" id="buyer_sku_code">
                        <option id="area_placeholder">--Pilih Area PDAM--</option>
                ...
                <div class="my-3 nm-inset-gray-200 rounded-lg p-1">
                    <input type="text"
                        ...
                        inputmode="numeric" pattern="[0-9]*" name="customer_no" id="customer_no" placeholder="No. PDAM"
                        autocomplete="off">
                </div>
...
<script>
    ...
    function check() {
        ...
        if (buyer_sku_code == '') {
            Swal.fire('Oops', 'Anda belum memilih Area PDAM', 'error');
            return false;
        }
        Swal.fire('Memproses...');
        Swal.showLoading();
