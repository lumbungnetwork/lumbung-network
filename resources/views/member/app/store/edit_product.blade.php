...
<div class="max-w-xs mx-auto">
    <div class="p-4">
        <div class="nm-flat-white rounded-2xl p-3">
            <div class="my-4 flex justify-end">
                <form id="delete-product-form"
                    action="{{ route('member.store.postDeleteProduct', ['product_id' => $product->id]) }}"
                    method="POST">
                    @csrf
                </form>
                <button id="delete-btn" type="button" style="font-size: 10px"
                    class="rounded-lg py-1 px-2 h-6 bg-gradient-to-br from-red-300 to-purple-300 font-light text-gray-700 outline-none focus:outline-none hover:shadow-lg hover:from-green-200 transition duration-200 ease-in-out">Hapus
                    Produk</button>
            </div>
            <form action="{{ route('member.store.postEditProduct', ['product_id' => $product->id]) }}" method="POST"
                enctype="multipart/form-data" id="add-product-form" autocomplete="off">
                @csrf
                <div class="my-3 text-xs text-gray-500">Ubah info produk anda:</div>
                ...
                <div class="mb-2 nm-inset-gray-100 rounded-xl p-2 overflow-x-scroll">
                    <input type="text" class="ml-2 w-full text-xs font-extralight bg-transparent focus:outline-none"
                        name="size" id="size" placeholder="1Kg, 1L, 500gr..." value="{{ $product->size }}">
                </div>
                @error('size')
                <div class="text-xs text-red-600 font-light">{{ $message }}</div>
                @enderror
                ...
                <div class="mb-2 nm-inset-gray-100 rounded-xl p-2 overflow-x-scroll">
                    <input type="text" inputmode="numeric" pattern="[0-9]*"
                        ...
                        name="price" id="price" placeholder="Hanya angka saja, cth: 25000"
                        value="{{ round($product->price) }}">
                </div>
                @error('price')
                <div class="text-xs text-red-600 font-light">{{ $message }}</div>
                @enderror
                <div style="font-size: 10px" class="text-gray-400 font-light">Jumlah Stock:</div>
                <div class="mb-2 nm-inset-gray-100 rounded-xl p-2 overflow-x-scroll">
                    <input type="text" inputmode="numeric" pattern="[0-9]*"
                ...
                <div class="mb-2 nm-inset-gray-100 rounded-xl p-2 overflow-x-scroll">
                    <textarea rows="3" class="bg-transparent w-full ml-2 text-xs font-extralight focus:outline-none"
                        name="desc" id="desc"
                        placeholder="Minyak Goreng berkualitas dengan 3x penyaringan...">{{ $product->desc }}</textarea>
                </div>
                @error('desc')
                <div class="text-xs text-red-600 font-light">{{ $message }}</div>
                @enderror
                ...
                <div id="search-image">
                    ...
                    <div class="mb-2 nm-inset-gray-100 rounded-xl p-2 overflow-x-scroll">
                        <input type="text" class="ml-2 w-full text-xs font-extralight bg-transparent focus:outline-none"
                            name="image" id="get-image" placeholder="ketikkan 3-4 huruf awal"
                            value="{{ $product->image }}">
                    </div>
                    @error('image')
                    <div class="text-xs text-red-600 font-light">{{ $message }}</div>
...
<script>
    ...
    $('#delete-btn').click( function () {
        Swal.fire({
            title: 'Hapus Produk?',
            text: "Apakah anda yakin ingin menghapus produk ini (permanen)?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
