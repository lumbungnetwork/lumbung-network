            ...
            <div class="px-6 py-6">
                ...
                <div class="mt-4 nm-convex-gray-50 rounded-xl p-6 ">
                    ...
                    <div class="mt-2 nm-inset-gray-50 rounded-xl p-2 ">
                        <input class="w-full focus:outline-none" type="text" placeholder="0 USDT" name="deposit"
                            id="deposit">
                    </div>
                ...
                <div class="mt-4 nm-concave-gray-50 rounded-xl p-6 text-center">
                    <p>USDT (Tether USD)</p>
                    <h2 class="mt-3 text-black text-6xl font-extralight">0.0</h2>
                    <div class="flex justify-center">
                        ...
                        <a href="/wallet/withdraw-usdt" type="button"
                            class="ml-2 mt-3 p-3 bg-gray-500 rounded-2xl w-24 text-white text-xs focus:outline-none focus:bg-gray-600">Withdraw</a>
                    </div>
...
<script>
    let usdtBalance = 0;
        ...
        $("#deposit-btn").click(async function () {
            ...
            const contract = tronWeb.contract(abi.entrys, USDTcontract);
            let res = contract.decodeInput($('#deposit').val());
            console.log(res);
            hash = '9ac23d5f5dee03e248b037e49c1aa33fbb963ce22df40e56b8d8d0eb0fc49f1c';

                Swal.fire('Validating, DO NOT CLOSE!');
                Swal.showLoading();
            ...
            // $.ajax({
            //     type: "POST",
            //     url: "{{ URL::to('/') }}/ajax/confirm-staking",
            //     data: {
            //     amount:amount,
            //     hash:hash,
            //     _token:_token
            ...
            // if (sendAmount > 0) {
            //     try {
            //         const resp = await contract.methods.transfer(toAddress, sendAmount).send();
        ...
        async function validateUSDTDeposit(data, hash) {
            const {
            abi
            } = await tronWeb.trx.getContract(USDTcontract);
            const contract = tronWeb.contract(abi.entrys, USDTcontract);
            const decodedData = await contract.decodeInput(data);
